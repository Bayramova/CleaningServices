const companies = [
  {
    id: "freshperspectives",
    logo: "./img/logo/atom-solid.svg",
    name: "Fresh Perspectives",
    address: "1600 Pennsylvania Avenue, Washington",
    rating: "4",
    orders: "50",
    servicesList: ["Standard Cleaning", "General Cleaning", "Repair Cleaning"],
    services: ["standardcleaning", "generalcleaning", "repaircleaning"],
    reviewsNumber: "25",
  },
  {
    id: "uborkakvartir",
    logo: "./img/logo/bath-solid.svg",
    name: "Uborka Kvartir",
    address: "11 Wall Street New York, NY",
    rating: "5",
    orders: "30",
    services: ["standardcleaning", "officecleaning", "furniturecleaning"],
    servicesList: ["Standard cleaning", "Office Cleaning", "Furniture Cleaning"],
    reviewsNumber: "25",
  },
  {
    id: "sparklingpalaces",
    logo: "./img/logo/candy-cane-solid.svg",
    name: "Sparkling Palaces",
    address: "350 Fifth Avenue New York, NY",
    rating: "4",
    orders: "77",
    services: ["industrialcleaning", "poolcleaning", "officecleaning"],
    servicesList: ["Industrial Cleaning", "Pool Cleaning", "Office Cleaning"],
    reviewsNumber: "30",
  },
  {
    id: "housecleaners",
    logo: "./img/logo/cat-solid.svg",
    name: "House Cleaners",
    address: "221 B Baker St, London",
    rating: "3",
    orders: "15",
    services: [
      "standardcleaning",
      "generalcleaning",
      "carpetcleaning",
      "furniturecleaning"
    ],
    servicesList: [
      "Standard Cleaning",
      "General Cleaning",
      "Carpet Cleaning",
      "Furniture Cleaning"
    ],
    reviewsNumber: "25",
  },
  {
    id: "rogaikopyta",
    logo: "./img/logo/frog-solid.svg",
    name: "Roga&Kopyta",
    address: "Tour Eiffel Champ de Mars, Paris",
    rating: "5",
    orders: "32",
    services: [
      "standardcleaning",
      "generalcleaning",
      "repaircleaning",
      "officecleaning",
      "carpetcleaning",
      "furniturecleaning",
      "industrialcleaning",
      "poolcleaning"
    ],
    servicesList: [
      "Standard Cleaning",
      "General Cleaning",
      "Repair Cleaning",
      "Office Cleaning",
      "Carpet Cleaning",
      "Furniture Cleaning",
      "Industrial Cleaning",
      "Pool Cleaning"
    ],
    reviewsNumber: "25",
  },
  {
    id: "cowboyscleaning",
    logo: "./img/logo/ghost-solid.svg",
    name: "Cowboys Cleaning",
    address: "4059 Mt Lee Dr. Hollywood, CA",
    rating: "4",
    orders: "30",
    services: ["carpetcleaning", "furniturecleaning"],
    servicesList: ["Carpet Cleaning", "Furniture Cleaning"],
    reviewsNumber: "25",
  },
  {
    id: "ramclean",
    logo: "./img/logo/hamburger-solid.svg",
    name: "RamClean Cleaning Services",
    address: "Buckingham Palace, London",
    rating: "3",
    orders: "10",
    services: ["repaircleaning", "industrialcleaning"],
    servicesList: ["Repair Cleaning", "Industrial Cleaning"],
    reviewsNumber: "25",
  },
  {
    id: "stormyscleaning",
    logo: "./img/logo/hippo-solid.svg",
    name: "Stormy's Cleaning Service",
    address: "Statue of Liberty, Liberty Island New York, NY",
    rating: "3",
    orders: "70",
    services: [
      "standardcleaning",
      "generalcleaning",
      "furniturecleaning",
      "carpetcleaning"
    ],
    servicesList: [
      "Standard Cleaning",
      "General Cleaning",
      "Furniture Cleaning",
      "Carpet Cleaning"
    ],
    reviewsNumber: "25",
  },
  {
    id: "aacleaning",
    logo: "./img/logo/horse-solid.svg",
    name: "A&A Cleaning Services",
    address: "Manager Square, Bethlehem, West Bank",
    rating: "4",
    orders: "66",
    services: ["standardcleaning", "poolcleaning"],
    servicesList: ["Standard Cleaning", "Pool Cleaning"],
    reviewsNumber: "25",
  },
  {
    id: "goclean",
    logo: "./img/logo/home-solid.svg",
    name: "GoClean",
    address: "2 Macquarie Street, Sydney",
    rating: "5",
    orders: "85",
    services: ["repaircleaning", "generalcleaning"],
    servicesList: ["Repair Cleaning", "General Cleaning"],
    reviewsNumber: "25",
  },
  {
    id: "chrisparklecleaning",
    logo: "./img/logo/globe-americas-solid.svg",
    name: "Chrisparkle Cleaning Services",
    address: "99, rue de Rivoli, Paris",
    rating: "4",
    orders: "20",
    services: ["standardcleaning", "generalcleaning", "poolcleaning"],
    servicesList: ["Standard Cleaning", "General Cleaning", "Pool Cleaning"],
    reviewsNumber: "25",
  },
  {
    id: "houseclechistogolikianers",
    logo: "./img/logo/dog-solid.svg",
    name: "Chistogoliki",
    address: "Piazza della Scala, Milano",
    rating: "3",
    orders: "60",
    services: [
      "standardcleaning",
      "generalcleaning",
      "repaircleaning",
      "officecleaning"
    ],
    servicesList: [
      "Standard Cleaning",
      "General Cleaning",
      "Repair Cleaning",
      "Office Cleaning"
    ],
    reviewsNumber: "25",
  },
  {
    id: "gryazinet",
    logo: "./img/logo/crow-solid.svg",
    name: "Gryazi net",
    address: "263 Prinsengracht, Amsterdam",
    rating: "3",
    orders: "10",
    services: [
      "standardcleaning",
      "generalcleaning",
      "carpetcleaning",
      "furniturecleaning"
    ],
    servicesList: [
      "Standard Cleaning",
      "General Cleaning",
      "Carpet Cleaning",
      "Furniture Cleaning"
    ],
    reviewsNumber: "25",
  },
  {
    id: "blesk",
    logo: "./img/logo/bread-slice-solid.svg",
    name: "Blesk",
    address: "1060 West Addison Street, Chigaco",
    rating: "5",
    orders: "20",
    services: [
      "standardcleaning",
      "generalcleaning",
      "carpetcleaning",
      "furniturecleaning",
      "poolcleaning"
    ],
    servicesList: [
      "Standard Cleaning",
      "General Cleaning",
      "Carpet Cleaning",
      "Furniture Cleaning",
      "Pool Cleaning"
    ],
    reviewsNumber: "25",
  },
  {
    id: "citycleaning",
    logo: "./img/logo/city-solid.svg",
    name: "City Cleaning",
    address: "10 Downing Street, London",
    rating: "4",
    orders: "30",
    services: ["repaircleaning", "officecleaning", "industrialcleaning"],
    servicesList: ["Repair Cleaning", "Office Cleaning", "Industrial Cleaning"],
    reviewsNumber: "25",
  },
  {
    id: "merrymaid",
    logo: "./img/logo/birthday-cake-solid.svg",
    name: "Merry Maids",
    address: "30 Rockefeller Plaza, Manhattan, NY",
    rating: "4",
    orders: "40",
    services: [
      "standardcleaning",
      "generalcleaning",
      "repaircleaning",
      "officecleaning",
      "carpetcleaning",
      "furniturecleaning",
      "industrialcleaning",
      "poolcleaning"
    ],
    servicesList: [
      "Standard Cleaning",
      "General Cleaning",
      "Repair Cleaning",
      "Office Cleaning",
      "Carpet Cleaning",
      "Furniture Cleaning",
      "Industrial Cleaning",
      "Pool Cleaning"
    ],
    reviewsNumber: "25",
  },
  {
    id: "mollymaid",
    logo: "./img/logo/baseball-ball-solid.svg",
    name: "Molly Maid",
    address: "6925 Hollywood Boulevard, Hollywood, CA",
    rating: "5",
    orders: "50",
    services: [
      "standardcleaning",
      "generalcleaning",
      "repaircleaning",
      "officecleaning",
      "carpetcleaning",
      "furniturecleaning",
      "industrialcleaning",
      "poolcleaning"
    ],
    servicesList: [
      "Standard Cleaning",
      "General Cleaning",
      "Repair Cleaning",
      "Office Cleaning",
      "Carpet Cleaning",
      "Furniture Cleaning",
      "Industrial Cleaning",
      "Pool Cleaning"
    ],
    reviewsNumber: "25",
  },
  {
    id: "freshcarpet",
    logo: "./img/logo/box-open-solid.svg",
    name: "Fresh Carpet",
    address: "Off A344 Road, Amesbury, Wiltshire",
    rating: "5",
    orders: "60",
    services: ["carpetcleaning", "furniturecleaning"],
    servicesList: ["Carpet Cleaning", "Furniture Cleaning"],
    reviewsNumber: "25",
  },
  {
    id: "krasniyuborshik",
    logo: "./img/logo/bread-slice-solid.svg",
    name: "Krasniy Uborshik",
    address: "Red Square, Moscow",
    rating: "3",
    orders: "70",
    services: ["officecleaning", "standardcleaning"],
    servicesList: ["Standard Cleaning", "Office Cleaning"],
    reviewsNumber: "25",
  },
  {
    id: "wendiscleaning",
    logo: "./img/logo/cogs-solid.svg",
    name: "Wendi's Cleaning Service",
    address: "Number 4, Privet Drive, Little Whinging",
    rating: "4",
    orders: "80",
    services: ["standardcleaning", "generalcleaning"],
    servicesList: ["Standard Cleaning", "General Cleaning"],
    reviewsNumber: "25",
  },
  {
    id: "residentialcleaning",
    logo: "./img/logo/dice-two-solid.svg",
    name: "Residential Cleaning Service",
    address: "42 Wallaby Way, Sydney",
    rating: "3",
    orders: "30",
    services: ["officecleaning"],
    servicesList: ["Office Cleaning"],
    reviewsNumber: "25",
  },
  {
    id: "hornshooves",
    logo: "./img/logo/cookie-bite-solid.svg",
    name: "Horns&Hooves",
    address: "11 Derby Street, Nawton",
    rating: "4",
    orders: "90",
    services: ["carpetcleaning", "furniturecleaning", "poolcleaning"],
    servicesList: ["Carpet Cleaning", "Furniture Cleaning", "Pool Cleaning"],
    reviewsNumber: "25",
  },
  {
    id: "hehhah",
    logo: "./img/logo/drumstick-bite-solid.svg",
    name: "Heh&Hah",
    address: "ul. Tolstogo 10, Minsk",
    rating: "5",
    orders: "35",
    services: ["repaircleaning", "industrialcleaning", "poolcleaning"],
    servicesList: ["Repair Cleaning", "Industrial Cleaning", "Pool Cleaning"],
    reviewsNumber: "25",
  },
  {
    id: "fish",
    logo: "./img/logo/fish-solid.svg",
    name: "Fish",
    address: "ul. Platonova 10, Minsk",
    rating: "4",
    orders: "40",
    services: ["repaircleaning", "industrialcleaning", "poolcleaning"],
    servicesList: ["Repair Cleaning", "Industrial Cleaning", "Pool Cleaning"],
    reviewsNumber: "25",
  }
];

export default companies;
